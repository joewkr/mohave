...
sd_create (SDId sd_id) sds_name data_type dim_sizes =
    ...
    withStaticVector dim_sizes $ \c_dim_sizes -> do
...
sd_getinfo sDataSetId@(SDataSetId sds_id) = do
  ...
  where
    ...
    emptySDataSetInfo = SDataSetInfoRaw "" 0 [] (TypedValue HNone ()) 0
...
@'Data.Format.HDF.LowLevel.Definitions.TypedValue' 'Data.Format.HDF.LowLevel.Definitions.HNone' ()@.
...
sd_diminfo sDimensionId@(SDimensionId dimension_id) = do
  ...
  where
    ...
    emptySDimensionInfo = SDimensionInfoRaw "" 0 (TypedValue HNone ()) 0
...
sd_attrinfo objId attrId =
  ...
  where
    ...
    emptySAttributeInfo = SAttributeInfoRaw "" 0 (TypedValue HNone ())
...
sd_getcal (SDataSetId sds_id) =
  ...
  where
    ...
    emptySCalibrationParameters = SCalibrationParametersRaw 1.0 0.0 0.0 0.0 (TypedValue HNone ())
...
sd_getdimscale sds sDimensionId@(SDimensionId dimension_id) = do
    (h_result_1, SDimensionInfoRaw{
        ...
        , sDimensionDataType=TypedValue{valueType=t :: HDataType dt}}) <- sd_diminfo sDimensionId
    ...
    if h_result_2 == (-1)
        then return (h_result_2, TypedValue HNone VS.empty)
        else case t of
            HNone -> return (h_result_2, TypedValue HNone VS.empty)
            _ -> do
                ...
                return $!
                    ...
                    , TypedValue t $ VS.unsafeFromForeignPtr0 fp (fromIntegral s))
...
sd_readattr objId attrId = do
    (h_result_1, SAttributeInfoRaw{
        ...
        , sAttributeDataType=TypedValue{valueType=t :: HDataType dt}}) <- sd_attrinfo objId attrId
    if h_result_1 == (-1)
        then return (h_result_1, TypedValue HNone VS.empty)
        else case t of
