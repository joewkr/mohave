...
nc_def_grp :: NC id -> String -> IO (CInt, NC GroupId)
nc_def_grp parentNcid name =
    ...
    alloca $ \groupNcidPtr -> do
        ...
        return $! (res, groupNcid)

nc_inq_dimids :: NC id -> Bool -> IO (CInt, [NCDimensionId])
...
nc_inq_grp_full_ncid :: NC id -> String -> IO (CInt, NC GroupId)
...
nc_inq_ncid :: NC id -> String -> IO (CInt, NC GroupId)
...
nc_inq_grp_ncid :: NC id -> String -> IO (CInt, NC GroupId)
nc_inq_grp_ncid = nc_inq_ncid

nc_inq_grp_parent :: NC GroupId -> IO (CInt, NC GroupId)
...
nc_inq_grpname :: NC id -> IO (CInt, String)
nc_inq_grpname ncid =
...
nc_inq_grpname_full :: NC id -> IO (CInt, String)
...
nc_inq_grpname_len :: NC id -> IO (CInt, Int)
nc_inq_grpname_len ncid =
    alloca $ \grpNameLenPtr -> do
        res <- c_nc_inq_grpname_len (ncRawId ncid) grpNameLenPtr
        grpNameLen <- peek grpNameLenPtr
        return $! (res, fromIntegral grpNameLen)

nc_inq_grps :: NC id -> IO (CInt, [NC GroupId])
...
nc_inq_varids :: NC id -> IO (CInt, [SomeNCVariable])
...
nc_rename_grp :: NC GroupId -> String -> IO (CInt, ())
