...
nc_inq_dim ncid (NCDimensionId dimid) =
    ...
    alloca $ \dimSizePtr -> do
        ...
        return $! (res, (dimName, fromIntegral dimSize))

nc_inq_dimid :: NC id -> String -> IO (CInt, NCDimensionId)
...
nc_inq_dimlen :: NC id -> NCDimensionId -> IO (CInt, Word32)
nc_inq_dimlen ncid (NCDimensionId dimid) =
    alloca $ \dimSizePtr -> do
        res <- c_nc_inq_dimlen (ncRawId ncid) dimid dimSizePtr
        dimSize <- peek dimSizePtr
        return $! (res, fromIntegral dimSize)

nc_inq_dimname :: NC id -> NCDimensionId -> IO (CInt, String)
...
nc_inq_ndims :: NC id -> IO (CInt, Int)
nc_inq_ndims ncid =
    alloca $ \numDimsPtr -> do
...
nc_inq_unlimdim :: NC id -> IO (CInt, NCDimensionId)
...
nc_rename_dim :: NC id -> NCDimensionId -> String -> IO (CInt, ())
